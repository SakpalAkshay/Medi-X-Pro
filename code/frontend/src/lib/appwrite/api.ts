...
export async function saveUserToDB(user: {
  ...
  try {
    console.log(user)
    if (user.usertype === 'patient' || user.usertype === "Patient"){
...
export async function getCurrentUser() {
  try {
    ...
    if (patientResponse.documents.length > 0) {
      return patientResponse.documents[0]; // Return the patient if found
    }

    // If not found, check in the doctor collection
    const doctorResponse = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.doctorCollectionId,
      [Query.equal("accountId", currentAccount.$id)]
    );
    ...
    if (doctorResponse.documents.length > 0) {
      return doctorResponse.documents[0]; // Return the doctor if found
    }
